...
func update_scroll_on_real() -> void:
	var container: Control = get_container()
	if !container: return
	scroll = -container.position - get_container_offset(content_scale)
...
func get_container_offset(scale_value: float) -> Vector2:
	...
	var container_size: Vector2 = (container.size + max_scroll_zone - min_scroll_zone) * scale_value
	return Vector2(
		get_position_on_flag(container.size_flags_horizontal, size.x, container_size.x),
		get_position_on_flag(container.size_flags_vertical, size.y, container_size.y)
	)
...
func minmax_scroll(value: Vector2, scale_value: float) -> Vector2:
	...
	var max_v: Vector2 = (container_size + max_scroll_zone) * scale_value - size
