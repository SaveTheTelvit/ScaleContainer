...
func _get_property_list() -> Array:
	return [
		...
		{"name": "Scale/scale", "type": TYPE_FLOAT, "usage": PROPERTY_USAGE_DEFAULT, "hint": PROPERTY_HINT_RANGE, "hint_string": "0.01,1000,0.01"},
		{"name": "Scale/min_scale", "type": TYPE_FLOAT, "usage": PROPERTY_USAGE_DEFAULT, "hint": PROPERTY_HINT_RANGE, "hint_string": "0.01,1000,0.01"},
		{"name": "Scale/max_scale", "type": TYPE_FLOAT, "usage": PROPERTY_USAGE_DEFAULT, "hint": PROPERTY_HINT_RANGE, "hint_string": "0.01,1000,0.01"},
		...
		{"name": "Controls/drag_alpha", "type": TYPE_FLOAT, "usage": PROPERTY_USAGE_DEFAULT, "hint": PROPERTY_HINT_RANGE, "hint_string": "0,1,0.01"},
		{"name": "Bound/hard_bounds", "type": TYPE_BOOL, "usage": PROPERTY_USAGE_DEFAULT, "hint": PROPERTY_HINT_NONE, "hint_string": ""},
		{"name": "Bound/returning_time", "type": TYPE_FLOAT, "usage": PROPERTY_USAGE_DEFAULT, "hint": PROPERTY_HINT_RANGE, "hint_string": "0,10,0.01"},
		{"name": "Bound/mouse_await_time", "type": TYPE_FLOAT, "usage": PROPERTY_USAGE_DEFAULT, "hint": PROPERTY_HINT_RANGE, "hint_string": "0,10,0.01"},
		{"name": "Bound/touch_await_time", "type": TYPE_FLOAT, "usage": PROPERTY_USAGE_DEFAULT, "hint": PROPERTY_HINT_RANGE, "hint_string": "0,10,0.01"}
	]
...
func _key_zoom(event: InputEventKey, point: Vector2, new_scale: float) -> void:
	if event.echo:
		...
		current_key_pressed = event.physical_keycode
	if !hard_bounds: returning_stop()
...
func set_view(new_scale: float, new_scroll: Vector2) -> void:
	...
	if !c: return
	c.position = -new_scroll + align_offset(new_scale)
	c.scale = Vector2(new_scale, new_scale)
...
func align_offset(scale_value: float) -> Vector2:
	...
	return Vector2(
		_align_axis(c.size_flags_horizontal, size.x, c.size.x * scale_value),
		_align_axis(c.size_flags_vertical, size.y, c.size.y * scale_value)
	)

func _align_axis(flag: int, avail: float, content: float) -> float:
	if flag & SIZE_SHRINK_END: return maxf(avail - content, 0.0)
	if flag & SIZE_SHRINK_CENTER: return maxf((avail - content) * 0.5, 0.0)
	return 0.0
...
func to_standard(await_time: float) -> void:
	...
	tween.chain().tween_method(_return_step, 0.0, 1.0, returning_time)

func _return_step(t: float) -> void:
	set_view(lerpf(return_from_scale, return_to_scale, t), return_from_scroll.lerp(return_to_scroll, t))
...
func update_scroll_on_real() -> void:
	var c: Control = get_container()
	if !c: return
	scroll = -c.position + align_offset(content_scale)
...
func minmax_scroll(value: Vector2, scale_value: float) -> Vector2:
	var c: Control = get_container()
	var cs: Vector2 = c.size if c else Vector2.ZERO
	var max_v: Vector2 = (cs + max_scroll_zone) * scale_value - size
	var min_v: Vector2 = min_scroll_zone * scale_value
	max_v = Vector2(maxf(max_v.x, min_v.x), maxf(max_v.y, min_v.y))
	return value.clamp(min_v, max_v)

func minmax_scale(value: float) -> float:
	if scale_on_content == ContentScale.NULL: return clamp(value, min_scale, max_scale)
	var c: Control = get_container()
	if !c: return 1.0
	var cs: Vector2 = c.size
	if cs.x == 0: cs.x = size.x
	if cs.y == 0: cs.y = size.y
	var ratio: Vector2 = size / cs
